...
class NoiseReductionEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None, clean_signal=None, noisy_signal=None):
        ...
        if clean_signal is None or noisy_signal is None:
            self.clean_signal = np.sin(np.linspace(0, 2*np.pi, 100))
            self.raw_signal = self.clean_signal + np.random.normal(0, 0.3, size=100)
        else:
            self.clean_signal = clean_signal
            self.raw_signal = noisy_signal
    ...
    def step(self, action):
        ...
        if snr_filtered < snr_raw:
            reward -= 0.1 * (self.threshold_factor ** 2)
        elif self.threshold_factor == 0:
            reward -= 0.05  # discourage turning off filtering completely

        reward = -1 if np.isnan(reward) else reward
